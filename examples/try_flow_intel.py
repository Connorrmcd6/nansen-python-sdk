...
try:
    ...
    for item in page.data[:5]:
        ...
        print("  Smart Traders:")
        print(f"    net_flow={fmt(item.smart_trader_net_flow_usd)}  wallets={item.smart_trader_wallet_count}")
        print("  Whales:")
        print(f"    net_flow={fmt(item.whale_net_flow_usd)}  wallets={item.whale_wallet_count}")
        print("  Exchanges:")
        print(f"    net_flow={fmt(item.exchange_net_flow_usd)}  wallets={item.exchange_wallet_count}")
        print("  Fresh Wallets:")
        print(f"    net_flow={fmt(item.fresh_wallets_net_flow_usd)}  wallets={item.fresh_wallets_wallet_count}")
